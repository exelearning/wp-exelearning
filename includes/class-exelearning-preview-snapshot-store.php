...
class ExeLearning_Preview_Snapshot_Store {
	...
	public function replace( $owner_id, $attachment_id, $zip_path, $preview_id = null ) {
		...
		// phpcs:disable WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents, WordPress.WP.AlternativeFunctions.file_system_operations_touch, WordPress.WP.AlternativeFunctions.rename_rename
		...
		$target = trailingslashit( $this->root ) . $id;
		...
		// phpcs:enable WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents, WordPress.WP.AlternativeFunctions.file_system_operations_touch, WordPress.WP.AlternativeFunctions.rename_rename
	...
	private function extract( $zip_path, $target ) {
		...
		for ( $index = 0; $index < $num_files; ++$index ) {
			...
			$operationsystem = 0;
			$attributes      = 0;
			if ( $zip->getExternalAttributesIndex( $index, $operationsystem, $attributes )
				&& ZipArchive::OPSYS_UNIX === $operationsystem && 0xa000 === ( ( $attributes >> 16 ) & 0xf000 ) ) {
				$zip->close();
			...
			$has_index = $has_index || 'index.html' === $name;
		}
		if ( ! $has_index || ! $zip->extractTo( $target ) ) {
			$zip->close();
	 ...
	 * @return bool
	 */
	private function reserved_path( $path ) {
