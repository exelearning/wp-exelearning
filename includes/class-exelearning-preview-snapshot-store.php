...
class ExeLearning_Preview_Snapshot_Store {
	...
	public function replace( $owner_id, $attachment_id, $zip_path, $preview_id = null ) {
		...
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents -- Private atomic store.
	...
	public function get( $preview_id, $path ) {
		...
		touch( trailingslashit( $root ) . '.accessed', call_user_func( $this->clock ) );
		return array(
			'path' => $file,
	...
	public function sweep_expired() {
		...
		foreach ( $entries ? $entries : array() as $id ) {
			...
			$accessed = @filemtime( trailingslashit( $this->root ) . $id . '/.accessed' );
	...
	private function metadata( $id ) {
		...
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- Private metadata file.
